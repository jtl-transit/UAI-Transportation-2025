...
class DataManager:
    ...
    def transform_wide_to_long(self, df_wide: pd.DataFrame = None,
        ...
        for _, row in df_wide.iterrows():
            ...
            chosen_alternative_raw = row['choice']
            
            # Convert choice to numeric if it's in string format (choice1 -> 1, choice2 -> 2)
            if isinstance(chosen_alternative_raw, str) and chosen_alternative_raw.startswith('choice'):
                chosen_alternative = int(chosen_alternative_raw.replace('choice', ''))
            else:
                chosen_alternative = chosen_alternative_raw
